function HullVertex = lineSearch(InitialPose, LowerBound, UpperBound, HullPrecision,...
...
% Else call function recursively
    ...
    r = m + lambda.*v0;
    pose_new = [r', R_vector];
    [~, ui] = kinematics.ik.standard(pose_new, FrameAnchors, PlatformAnchors, '1R2T');
    [AT, ~] = kinetostatics.structm.m1R2T(FrameAnchors, ui, R_matrix);
   % f = kinetostatics.fdist.improved_closed_form(wp, AT, fmin, fmax);
    ...
    r = workspace.lineSearch(aPose, lb, ub, epsilon, v0, aFrame, aPlatform, wp, fmin, fmax);
end
