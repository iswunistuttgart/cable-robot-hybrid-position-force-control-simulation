function [Distribution] = improved_closed_form(Wrench, StructureMatrix, ForceMinimum, ForceMaximum) %#codegen
...
% validateattributes(Wrench, {'numeric'}, {'vector', 'numel', 6, 'finite'}, mfilename, 'Wrench', 1);
...
if math.issquare(aStructureMatrixAt)
    vForceDistribution = aStructureMatrixAt\(-vWrench);
% Non standard case, where we have more cables than degrees of freedom
else
    vForceDistribution = kinetostatics.fdist.closed_form(vWrench, aStructureMatrixAt, vForceMinimum, vForceMaximum);
    ...
    if iViolationIndex > 0
        ...
        vReducedForceDistribution = kinetostatics.fdist.improved_closed_form(vReducedWrench, aReducedStructureMatrixAt, vReducedForceMinimum, vReducedForceMaximum);
